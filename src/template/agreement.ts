...
export const agreementHtml = async (demand: Partial<Demand>): Promise<string> => {
  return `
    <div
      style="
    ...
    demand.defendant?.lastName ?? ""
          ...
          del Supremo Tribunal de Justicia del Estado de Sonora el correo electrónico ${demand?.defendant?.email ?? ""} en cual deberán hacerse las notificaciones durante la vigencia de la contingencia sanitaria, personalidad
  ...
  )})</b> el cual fue suscrito por <b>C. ${demand.defendant?.name ?? ""} ${
    demand.defendant?.lastName ?? ""
  ...
  directa en contra del demandado.
    ...
    demand.defendant?.lastName ?? ""
  }</b> en su carácter de deudor a <b>${demand.actor?.name ?? ""} ${
    demand.actor?.lastName ?? ""
      ...
      <div style="display: grid; page-break-inside: avoid;">
        <p style="text-align: justify; text-justify: inter-word; margin-top: 10px; text-indent: 5%; page-break-inside: avoid;">
          <b>TERCERA.-</b> Declara el <b>C. ${demand.defendant?.name ?? ""} ${
    demand.defendant?.lastName ?? ""
          ...
          de cedula, ${(demand.freeText != "" && demand.freeText != null) ? demand.freeText + ", " : ""}copia del documento base de la acción consistente en un pagaré con su respectivo endoso, copia del 
          Registro Federal de Contribuyente (RFC) del actor(a),
          ${
            (demand.actor?.economicActivity ?? "Persona Fisica") == "Persona Fisica"
              ? ` copia de la Clave Única de Registro de Población (CURP) del actor(a),`
              : ``
          }
           y copia de credencial para votar con fotografía del actor(a), y copia del presente convenio; 
    ...
    demand.defendant?.lastName ?? ""
  }</b> pagará a <b>C. ${demand.actor?.name ?? ""} ${
    demand.actor?.lastName ?? ""
...
<b>${NumberToLetter(
    ...
    ((demand.defendant?.position ?? "") == "OTRO" ? demand.defendant?.positionOther : (demand.defendant?.position ?? ""))
  } ${(demand.defendant?.employeeNo ?? "")}</b> debiéndose girar los correspondientes oficios con los insertos necesarios a <b>${
    ((demand.paymentTextAgreement ?? "") == "OTRO" ? demand.paymentTextAgreementOther : (demand.paymentTextAgreement ?? ""))
 ...
 <b>Número de Tarjeta ${demand.actor?.cardNumber ?? ""}</b> del banco ${
    demand.actor?.bank ?? ""
  } a nombre de <b>C. ${demand.actor?.name ?? ""} ${
    demand.actor?.lastName ?? ""
 ...
 regulándose los intereses moratorios a razón del ${await getByCode("PORCENTAJE_CONVENIO")} anual.
        ...
        <p style="text-align: justify; text-justify: inter-word; margin-top: 10px; text-indent: 5%;">
          Por parte del demandado el ubicado en <b>${ (demand.exhorted == false ? `${
