...
export const welcomeHtml = async (user: Partial<User>, password: string): Promise<string> => {
    return `
        <div style="display: flex; text-align:center; margin-inline: 20px;">
          <h3><b>¡Gracias por tu confianza y apoyo!</b></h3><br/>
          <h3><b>Usuario:</b> ${user.user}</h3><br/>
          <h3><b>Contraseña</b> ${password}</h3><br/>
          ...
          <a href="https://perfil.mexicof3.com/">MexicoF3<a>
        </div>
    `
} 